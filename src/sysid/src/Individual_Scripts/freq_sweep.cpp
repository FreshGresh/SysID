...
int main(int argc, char **argv) {
	...
	doc.open("/home/nsl/sysid3_ws/src/sysid/src/freqswlog_input_100Hz.csv");// loads .csv file
	vector<double> t, dCS;// initializes arrays to store values from csv file. t -> time, dA-> aileron command, dE -> elevator command, dR-> rudder command
	...
	while (line.length() > 0) {//This while loop extracts the data from the .csv file and stores it in its corresponding arrays. The while condition checks if the line actually has any values
		...
		ROS_INFO_STREAM("t = " + to_string(t.back()) + ", dCS = " + to_string(dCS.back()) ); // Prints the last datapoints successfully extracted (for debugging purposes only)
		counter++;}//increments the counter to keep track of how many steps imported from file
	...
	int thresh_1 = (PWM_max - PWM_min)/3 + PWM_min; // The first threshold for channel 6 to select which control surface the frequency sweep passes through
	int thresh_2 = (PWM_max - PWM_min)/3 + thresh_1; // The second threshold for channel 6 to select which control surface the frequency sweep passes through
	double amp = 0.6; // Frequency sweep gain
	int phase = 0;// The phase of the maneuver. 0 when off and 1 when it is being executed
	double dA_com = 0.0, dE_com = 0.0, dR_com = 0.0, com = 0.0;// Initializes the commanded actuator inputs
	...
	while(ros::ok()) {
		...
		switch (phase) {
			...
			case 1://The "on" phase: the script sequentially executes the pre-scripted multisine maneuver. Flipping the switch of channel 5 terminates and resets the maneuver. Channel 6 controls the gain of the multisine
				...
				if (counter > count_max || trigger < 1500){ //Checks for either of the conditions which would terminate the maneuver: Whether the time elapsed exceeds the total time of the maneuver or whether the switch for channel 5 have been flipped to the "off" setting
		...
		if (PWM > thresh_2){
			dR_com = com;} // Passes command to the rudder if channel 6 is between the second threshold and maximum
		else if (PWM > thresh_1){
			dE_com = com;} // Passes command to the elevators if channel 6 is between the two thresholds
		else{
			dA_com = com;} // Passes command to ailerons if channel 6 is between the minimum and the first threshold
		ROS_INFO_STREAM(to_string(PWM));
