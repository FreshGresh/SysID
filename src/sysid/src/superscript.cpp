...
int length_m = 0;// variable to keep track of the length of the multisine vectors
void load_multisine(){
	ifstream doc;// Create variable to load .csv file
	//doc.open("/home/nsl/sysID/src/sysid/src/ms_input_100Hz.csv");// loads .csv file
	doc.open("/home/nsl/sysID/src/sysid/src/InputCSVs/ms_input_100Hz_0.1HzMin_2.0HzMax_aero.csv");// loads .csv file for aerodynamic modeling
	...
	while (line.length() >0) {//This while loop extracts the data from the .csv file and stores it in its corresponding arrays. The while condition checks if the line actually has any values
		...
		length_m++;}//increments the counter in preparation for reading the following line and storing its enclosed values in the next position along the arrays
}
...
/* */ //Square wave mapping input initialization
vector<double> t_map, dCS_map;// initializes arrays to store values from csv file. t -> time, dCS-> control surface command
int length_map = 0;// variable to keep track of the length of the frequency sweep  vectors
void load_map(){
	ifstream doc;// Create variable to load .csv file
	doc.open("/home/nsl/sysID/src/sysid/src/InputCSVs/sqwv_map_input_servo.csv");// loads .csv file for servo pwm mapping input
	string line, word, temp;// creates string variables
	getline(doc,line);// reads entire row and stores it in variable line and the removes that row from the variable doc
	while (line.length() > 0) {//This while loop extracts the data from the .csv file and stores it in its corresponding arrays. The while condition checks if the line actually has any values
		stringstream s(line);//s is used to break line up into its individual "words" (numbers in string format)
		getline(s, word, ','); // extracts string from s (line) until it hits the delimiter (comma in this case). It then stores the string in word and removes comma
		t_map.push_back(stod(word));// converts the string stored in word to a double and appends it to the end of its respective vector
		getline(s, word, '\n'); // extracts string from s (line) until it hits the delimiter (end of line in this case). It then stores the string in word
		dCS_map.push_back(stod(word));// converts the string stored in word to a double and appends it to the end of its respective vector
		getline(doc,line);// reads the next row row and stores it in variable line and the removes that row from the variable doc
		length_map++;}//increments the counter to keep track of how many steps imported from file
	doc.close(); // Clearing variable since we no longer need it
}  //end square wave for servo mapping


//Square Wave 3-2-1-1 Initialization
vector<double> t_sw, dA_sw, dE_sw, dR_sw;// initializes arrays to store values from csv file. t -> time, dA-> aileron command, dE -> elevator command, dR-> rudder command
int length_sw = 0;// variable to keep track of the length of the square wave vectors
void load_squarewave(){
	ifstream doc;// Create variable to load .csv file
	doc.open("/home/nsl/sysID/src/sysid/src/sqw_3axis_input.csv");// loads .csv file
	string line, word, temp;// creates string variables
	getline(doc,line);// reads entire row and stores it in variable line and the removes that row from the variable doc
	while (line.length() >0) {//This while loop extracts the data from the .csv file and stores it in its corresponding arrays. The while condition checks if the line actually has any values
		stringstream s(line);//s is used to break line up into its individual "words" (numbers in string format)
...
} // End Function to execute multisine


// Function to execute frequency sweep
...
} // End Function to execute frequency sweep


/* */ // Function to execute square wave for servo mapping
void servo_map(ros::Rate rate){
	mavros_msgs::ActuatorControl act_con;// Initializes variable used to actuate servos
	double t0 = 0;// Records initial time
	double dt = 0.01; // Length of time step of maneuver
	int phase = 0;// The phase of the maneuver. 0 when off and 1 when it is being executed
	...
	while(ros::ok() && mode < 1333){
		...
		switch (phase) {
			case 0:// The "off" phase: the script merely passes pilot's manual inputs. If switch for channel 5 is flipped, it resets initial time, intiializes maneuver and transitions to phase 1
				...
				break;
			case 1://The "on" phase: the script sequentially executes the pre-scripted maneuver. Flipping the switch of channel 5 terminates and resets the maneuver. Channel 6 controls the gain of the multisine
				while ( t_map[counter] + dt + t0 < ros::Time::now().toSec() && counter <= length_map) { //Assumes the code's refresh rate is significantly faster than the prescribed time step of maneuver. Checks if the time elapsed since the start of the maneuver has exceeded the next time step. If so it updates the counter until it reaches the position of the highest time step during the maneuver less than the elapsed time
					counter++;} // Updates counter
				if (counter > length_map || PTI < 1500){ //Checks for either of the conditions which would terminate the maneuver: Whether the time elapsed exceeds the total time of the maneuver or whether the switch for channel 5 have been flipped to the "off" setting
					com = 0.0; // resets the deflection to zero
					phase = 0;} // Resets the phase to "off"
				else {
					com = dCS_map[counter];} // Passes the desired deflection at the corresponding time step
				break;
		}
			
			//dT_com = com; //Use this line for throttle input, otherwise, use lines below
		 //Comment out this section for throttle input
		if (submode > 1667){
			dE_com = com;} // Passes command to the rudder if channel 6 is between the second threshold and maximum
		else if (submode > 1333){
			dR_com = com;} // Passes command to the elevators if channel 6 is between the two thresholds
			...
			dA_com = com;} // Passes command to ailerons if channel 6 is between the minimum and the first threshold
         // end of commented secion for throttle input
...
void doublet(ros::Rate rate)
{
	...
	while(ros::ok() && mode >= 1667){
        	...
        	if (submode > 1666){//Longitudinal doublet maneuver
                t = 0.8;//elevator doublet period - ie short period
                t1 = t/2; t2 = t/2;
                ...
                t = 1.2;//rudder doublet period - ie dutch roll period
                t1 = t/2; t2 = t/2;
  ...
  // 3-2-1-1 commented out if propulsion testing
		...
		switch (phase) {
			case 0:// The "off" phase: the script merely passes pilot's manual inputs. If switch for channel 5 is flipped, it resets initial time, intiializes maneuver and transitions to phase 1
				...
				if (PTI > 1500){ //Condition to check if the switch for channel 5 is flipped
					...
					counter = 0; //sets the counter of the sequence to zero (uses same counter used to extract data from the .csv file
					phase = 1; //Changes the phase to "on"
					dA_com = dA_sw[0] * amp; //assigns initial demanded aileron deflection of the maneuver 
					dE_com = dE_sw[0] * amp; //assigns initial demanded elevator deflection of the maneuver
					dR_com = dR_sw[0] * amp;} //assigns initial demanded rudder deflection of the maneuver
				break;
			case 1://The "on" phase: the script sequentially executes the pre-scripted multisine maneuver. Flipping the switch of channel 5 terminates and resets the maneuver. Channel 6 controls the gain of the multisine
				...
				if (counter > length_sw - 1 || PTI < 1500){ //Checks for either of the conditions which would terminate the maneuver: Whether the time elapsed exceeds the total time of the maneuver or whether the switch for channel 5 have been flipped to the "off" setting
					...
					dA_com = dA_sw[counter] * amp; // Passes the desired aileron deflection at the corresponding time step
					dE_com = dE_sw[counter] * amp; // Passes the desired elevator deflection at the corresponding time step
					dR_com = dR_sw[counter] * amp;} // Passes the desired rudder deflection at the corresponding time step
				break;
		}
...
int main(int argc, char **argv)
{
    ...
    //execute the functions that load the .csv vectors to RAM
	load_multisine();
	load_freq_sweep();
	load_squarewave();
	load_map(); 
    ...
    while(ros::ok()){
		if(mode < 1333)
			//multisine(rate); // comment out for servo mapping
			servo_map(rate); // servo_map commented out unless mapping servos
		else if (mode < 1666)
			freq_sweep(rate);
		else
			doublet(rate);
